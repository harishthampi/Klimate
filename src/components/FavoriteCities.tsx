...
const FavoriteCities = () => {
  const { favoritesQuery, removeFavorites } = useFavorites();
    ...
    <>
      ...
      <ScrollArea className="w-full pb-4">
        <div className="flex gap-4">
          {favoritesQuery.data?.map((city: FavoriteCityProps) => (
            <FavoriteCity
              key={city.id}
              {...city}
...
const FavoriteCity = ({ id, name, lat, lon, onRemove }: FavoriteCityProps) => {
  ...
  return (
    ...
    gap-3 rounded-lg border bg-card p-4 pr-8 shadow-sm hover:shadow-md transition-all"
    role="button"
    tabIndex={0}
    >
